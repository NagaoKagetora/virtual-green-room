#include "CurvatureColorizationProvider.hpp"
#include <osg/Vec4>
#include <common/util/logger.hpp>



bool CurvatureColorizationProvider::compute(Model *source, Model *target) {
	...
	vcg::tri::UpdateCurvature<pcvcg::TriMesh>::MeanAndGaussian(*(source->mesh()));
	vcg::tri::UpdateCurvature<pcvcg::TriMesh>::MeanAndGaussian(*(target->mesh()));
	...
	for (size_t i = 0; i < size; ++i) {
		real_t dist = target->mesh()->vert[i].cKh() - source->mesh()->vert[i].cKh();
