#include "NComponentColorizationProvider.hpp"
#include <osg/Vec4>
#include <common/util/logger.hpp>



bool NComponentColorizationProvider::compute(Model *source, Model *target) {
	...
	for (size_t i = 0; i < size; ++i) {
		real_t dist = target->mesh()->vert[i].cN() * (target->mesh()->vert[i].cP() - source->mesh()->vert[i].cP());
